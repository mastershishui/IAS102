...
// Add these lines to connect the new buttons
    const unlockUsersBtn = document.getElementById("unlockUsersBtn");
    if (unlockUsersBtn) unlockUsersBtn.addEventListener("click", showUnlockUsersForm);
    
    const viewLockedBtn = document.getElementById("viewLockedBtn");
    if (viewLockedBtn) viewLockedBtn.addEventListener("click", showViewLockedForm);
...
function initEmployeeRecordPage() {
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        if (btnAdd)  btnAdd.style.display  = perms.add  ? "inline-block" : "none";
        if (btnEdit) btnEdit.style.display = perms.edit ? "inline-block" : "none";
        if (btnView) btnView.style.display = perms.view ? "inline-block" : "none";
    });
}
...
// -----------------------------------------------------
function showEditEmployeesForm() {
    const panel = document.getElementById("um-content");
    ...
    panel.innerHTML = `
        ...
        <div id="editFormContainer" class="um-edit-form" style="display: none;">
            ...
            <div class="form-row">
                <label>Role:</label>
                <select id="editRole" style="width:260px; height:34px; border-radius:8px; border: 1px solid #8b6f61;">
                    <option value="regular">User</option>
                    <option value="admin">Admin</option>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <select id="editStatus" style="width:260px; height:34px; border-radius:8px; border: 1px solid #8b6f61;">
            ...
            <div class="form-row">
                ...
                <select id="editLockStatus" style="width:260px; height:34px; border-radius:8px; border: 1px solid #8b6f61;">
...
function displayEmployees(querySnapshot) {
    ...
    querySnapshot.forEach(doc => {
        ...
        const statusText = data.locked ? "Locked" : (data.active ? "Active" : "Inactive");
...
async function loadEmployeeForEdit(username) {
    try {
        ...
        document.getElementById("editRole").value = data.role || "regular";
        document.getElementById("editStatus").value = data.active ? "active" : "inactive";
        document.getElementById("editLockStatus").value = data.locked ? "locked" : "unlocked";
...
async function saveEmployeeEdit(oldUsername) {
    ...
    const active = document.getElementById("editStatus").value === "active";
    const locked = document.getElementById("editLockStatus").value === "locked";
    ...
    try {
        ...
        // Reset failed attempts if unlocking account
        if (locked === false) {
        ...
        <div id="resetPasswordForm" class="um-edit-section" style="display: none;">
            ...
            <div class="form-row">
                ...
                <div style="display: flex; flex-direction: column; width: 280px;">
                    <input type="text" id="newPasswordReset" style="width: 100%;">
                    <button id="generatePasswordBtn" class="um-submit-btn" style="margin-top: 10px; padding: 8px 15px; font-size: 16px;">
            ...
            <div class="form-row">
                ...
                <input type="text" id="confirmPasswordReset" style="width: 280px;">
            </div>
            
            <div class="form-row">
                <label>Require Password Change:</label>
                <select id="requirePasswordChange" style="width: 280px; height: 38px; border-radius: 8px; border: 2px solid #8b6f61; padding: 0 10px;">
                    <option value="yes">Yes - User must change password on next login</option>
                    <option value="no">No - Keep current settings</option>
                </select>
            </div>
            
            <div class="form-row">
                <label>Send Email Notification:</label>
                <select id="sendEmailNotification" style="width: 280px; height: 38px; border-radius: 8px; border: 2px solid #8b6f61; padding: 0 10px;">
                    <option value="yes">Yes - Send new password via email</option>
...
function showUserResetForm(username, email) {
    ...
    resetForm.setAttribute("data-username", username);
    resetForm.setAttribute("data-email", email);
    ...
    // Set default values
    document.getElementById("requirePasswordChange").value = "yes";
    document.getElementById("sendEmailNotification").value = "yes";
    
    // Scroll to reset form
    resetForm.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    
    // Add event listeners
    document.getElementById("confirmResetBtn").onclick = () => confirmPasswordReset(username, email);
...
function generatePasswordForReset() {
    ...
    document.getElementById("newPasswordReset").value = strongPassword;
    document.getElementById("confirmPasswordReset").value = strongPassword;
}
...
async function confirmPasswordReset(username, email) {
    ...
    const requirePasswordChange = document.getElementById("requirePasswordChange").value === "yes";
    const sendEmailNotification = document.getElementById("sendEmailNotification").value === "yes";
    ...
    try {
        // Note: In a real app, you would need to use Firebase Admin SDK on a server
        // to reset another user's password. The client SDK doesn't allow this directly.
        // This is a simplified implementation for demonstration.
        
        // For now, we'll just update the Firestore document and show a message
        // In a real app, you would need to implement a server-side function
        
        // Update user document with firstLogin flag if required
        const updateData = {};
        ...
        alert(`Password reset for ${username} has been scheduled.\n\n` +
              `New Password: ${newPassword}\n\n` +
              `IMPORTANT: In a real application, you would need to use Firebase Admin SDK ` +
              `on a server to actually reset the password. This is a demonstration only.`);
        
        // Hide the reset form
        document.getElementById("resetPasswordForm").style.display = "none";
        
        // Clear search and reload
        document.getElementById("searchResetUser").value = "";
        searchResetUsers();
    ...
    panel.innerHTML = `
        ...
        <div id="activateDeactivateForm" class="um-edit-section" style="display: none;">
            ...
            <div class="form-row">
                ...
                <textarea id="statusChangeReason" rows="3" style="width: 280px; border-radius: 8px; border: 2px solid #8b6f61; padding: 10px; font-family: Arial; font-size: 16px;" 
                          placeholder="Optional: Enter reason for status change..."></textarea>
            </div>
            
            <div class="form-row">
                <label>Notify User:</label>
                <select id="notifyUser" style="width: 280px; height: 38px; border-radius: 8px; border: 2px solid #8b6f61; padding: 0 10px;">
...
function showUnlockUsersForm() {
    ...
    panel.innerHTML = `
        ...
        <div id="unlockForm" class="um-edit-section" style="display: none;">
            ...
            <div class="form-row">
                <label>Notify User:</label>
                <select id="notifyUnlock" style="width: 280px; height: 38px; border-radius: 8px; border: 2px solid #8b6f61; padding: 0 10px;">
                    <option value="yes">Yes - Send unlock notification email</option>
                    <option value="no">No - Don't send notification</option>
                </select>
            </div>
